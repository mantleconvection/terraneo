...
double test( int level, util::Table& table )
{
    ...
    **/
    ...
    auto g        = linalg::allocate_vector_q1_vec< ScalarType, 3 >( "g", domain, level );
    auto Adiagg   = linalg::allocate_vector_q1_vec< ScalarType, 3 >( "Adiagg", domain, level );
    auto tmp      = linalg::allocate_vector_q1_vec< ScalarType, 3 >( "tmp", domain, level );
    auto solution = linalg::allocate_vector_q1_vec< ScalarType, 3 >( "solution", domain, level );
    auto error    = linalg::allocate_vector_q1_vec< ScalarType, 3 >( "error", domain, level );
    auto b        = linalg::allocate_vector_q1_vec< ScalarType, 3 >( "b", domain, level );
    auto r        = linalg::allocate_vector_q1_vec< ScalarType, 3 >( "r", domain, level );
    ...
    const auto num_dofs = 3 * kernels::common::dot_product( mask_data_long, mask_data_long );
    ...
    if ( true )
    {
        vtk::VTKOutput vtk_after(
            ...
            subdomain_radii,
            "vector_laplace_cg_level" + std::to_string( level ) + ".vtu",
            false );
        vtk_after.add_vector_field( g.grid_data( level ) );
        vtk_after.add_vector_field( u.grid_data( level ) );
        vtk_after.add_vector_field( solution.grid_data( level ) );
        vtk_after.add_vector_field( error.grid_data( level ) );
