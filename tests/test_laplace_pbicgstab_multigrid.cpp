    ...
    test( int min_level, int max_level, const std::shared_ptr< util::Table >& table, double omega, int prepost_smooth )
{
    ...
    std::vector< VectorQ1Scalar< ScalarType > > tmp_bicgstab;
    ...
    for ( int level = min_level; level <= max_level; level++ )
    {
        ...
        if ( level == max_level )
        {
            for ( int i = 0; i < 8; ++i )
            {
                tmp_bicgstab.emplace_back( "tmp_bicgstab" + std::to_string( level ), domains[level], mask_data[level] );
            }
        }
    }
    ...
    linalg::solvers::IterativeSolverParameters solver_params_pbicgstab{ 20, 1e-12, 1e-16 };

    linalg::solvers::PBiCGStab< Laplace, Multigrid > pbicgstab_solver(
        2, solver_params_pbicgstab, table, tmp_bicgstab, multigrid_solver );

    Kokkos::fence();
    Kokkos::Timer timer;
    timer.reset();
    linalg::solvers::solve( pbicgstab_solver, A, u, f );
    Kokkos::fence();
